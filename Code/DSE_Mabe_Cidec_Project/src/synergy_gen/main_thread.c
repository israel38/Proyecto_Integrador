...
static uint8_t main_thread_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.main_thread") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
#if (BSP_IRQ_DISABLED) != BSP_IRQ_DISABLED
#if !defined(SSP_SUPPRESS_ISR_g_transfer2) && !defined(SSP_SUPPRESS_ISR_DTCELC_EVENT_SCI0_TXI)
...
static const external_irq_cfg_t g_touch_irq_cfg = { .channel = 9, .trigger =
		EXTERNAL_IRQ_TRIG_FALLING, .filter_enable = true, .pclk_div =
		EXTERNAL_IRQ_PCLK_DIV_BY_64, .autostart = true, .p_callback = NULL,
		.p_context = &g_touch_irq, .p_extend = NULL, .irq_ipl = (3), };
/* Instance structure to use this module. */
const external_irq_instance_t g_touch_irq = { .p_ctrl = &g_touch_irq_ctrl,
		.p_cfg = &g_touch_irq_cfg, .p_api = &g_external_irq_on_icu };
sf_external_irq_instance_ctrl_t g_sf_touch_irq_ctrl;
const sf_external_irq_cfg_t g_sf_touch_irq_cfg = { .event =
		SF_EXTERNAL_IRQ_EVENT_SEMAPHORE_PUT, .p_lower_lvl_irq = &g_touch_irq, };
/* Instance structure to use this module. */
const sf_external_irq_instance_t g_sf_touch_irq = { .p_ctrl =
		&g_sf_touch_irq_ctrl, .p_cfg = &g_sf_touch_irq_cfg, .p_api =
...
#undef SYNERGY_NOT_DEFINED	
		...
		.txi_ipl = (3), .tei_ipl = (3), .eri_ipl = (3), .p_extend = NULL, };
/* Instance structure to use this module. */
...
const sf_touch_panel_i2c_cfg_t g_sf_touch_panel_i2c_cfg_extend = {
		.p_lower_lvl_i2c = &g_i2c, .p_lower_lvl_irq = &g_sf_touch_irq, .pin =
...
void main_thread_create(void) {
	...
	err = tx_thread_create(&main_thread, (CHAR *) "Main Thread",
			main_thread_func, (ULONG) NULL, &main_thread_stack, 1024, 6, 6, 10,
			TX_AUTO_START);
	if (TX_SUCCESS != err) {
